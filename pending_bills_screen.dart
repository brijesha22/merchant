...
class _BillListState extends State<BillList> {
  ...
  void _showSettleBillDrawer(BuildContext context, String billno, double amount) async {
    ...
    List<Settlement> settlementList = await fetchSettlementList(); // <- your new API call
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        return SettleBillDrawer(
          billId: billno,
          amount: amount,
          settlements: settlementList, // <- use API-driven names
      ...
      BuildContext context, String billId, double billAmount)
  {
    ...
    givenAmountController.addListener(() {
      ...
      refundAmount > 0 ? refundAmount.toStringAsFixed(3) : '0.00';
    });
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                'Bill Amount: ₹${billAmount.toStringAsFixed(3)}',
    ...
    FlutterMosambeeAar.onResult.listen((resultData) async {
      if (resultData.result != null && resultData.result == true) {
        ...
        if (currentTask == "UPI QR") {
          ...
          _updateSettlementMode(billNoValue, "Card"); // ✅ Now updates correctly
        }
...
/*    bytes += generator.row([
    ...
    bytes += generator.row([
      ...
      PosColumn(
          text: grandtotal.toStringAsFixed(3)+'  ',
          width: 4,
          styles: const PosStyles(fontType: PosFontType.fontB,
            bold: false,
            height: PosTextSize.size2,
                                ...
                                title: Row(
                                  ...
                                  children: [
                                    ...
                                    Checkbox(
                                      ...
                                      // Bind the checkbox value to the list of selected items
                                    ...
                                    /*Checkbox(
                                          value: _selectedItems[index],  // Bind the checkbox value to the list of selected items
                                          onChanged: (bool? value) {
                                            setState(() {
                                              // Update the selected state for this checkbox
                                              _selectedItems[index] = value ?? false;
                                             // isCheckBox=value??false;
                                              // Print the updated value to see if it works
                                              print('Checkbox at index $index is now: ${_selectedItems[index]}');
                                            });
                                          },
                                        )*/
                                  ],
                                  ...
                                  children: [
                                    Column(
                                      ...
                                      children: [
                                        ...
                                        const SizedBox(height: 4),
                                        Text(
                                          'Amount: ${finalAmount.toStringAsFixed(3)}',
                                          style: TextStyle(
                                            color: Color(0xFFD5282A),
                                            fontWeight: FontWeight.bold,
                                          ),
                                                ...
                                                if (selectedOption == 'Tax') {
                                                  ...
                                                  if (discountPercent != null) {
                                                    try {
                                                      ...
                                                      await showSuccessDialog(context, billNo);
                                                      await executeReprintLogic(snapshot.data![index]); // ✅ Added reprint logic after success
                                                    } catch (e) {
                                                      ScaffoldMessenger.of(context).showSnackBar(
              ...
              child: Padding(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    // Now you have the selectedBills list, and you can use it
                    // For example, you can pass it to a method or display a dialog
                    if (selectedBills.isNotEmpty) {
                      // Call a function to handle the selected bills, like:
                      //_showSettleBillDrawer(context, selectedBills[i].billNo.toString(), finalAmount);
                    } else {
                      // Show a message or handle case where no bills are selected
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                            content: Text(
  ...
  void viewBill(String billNo, String tableNumber, String waiterName) async {
    try {
      ...
      String _extractFormattedTime(String? rawTime) {
        ...
        try {
          final dt = DateTime.parse(rawTime).toLocal(); // Convert to local time
          return DateFormat('hh:mm a').format(dt); // Format as 04:19 PM
        } catch (e) {
    ...
    return InkWell(
      onTap: () async {
        ...
        bool transactionSuccessful =
            await performTransaction(settlementMode, amount) ?? false;
